...
function jupiterx_core_add_custom_widget_area() {
	...
	$sidebars = get_option( 'jupiterx_custom_sidebars' );
	...
	if ( empty( $_POST['name'] ) ) {
		wp_send_json_error( __( 'name field is required', 'jupiterx' ) );
	}

	$name       = sanitize_text_field( wp_unslash( $_POST['name'] ) );
	$sidebars[] = [ 'name' => $name ];

	update_option( 'jupiterx_custom_sidebars', $sidebars );
...
function jupiterx_core_delete_custom_widget_area() {
	...
	if ( empty( $_POST['id'] ) && '0' !== $_POST['id'] ) { // phpcs:ignore WordPress.Security
		wp_send_json_error( __( 'id field is required', 'jupiterx' ) );
	}
...
add_action( 'widgets_admin_page', function () { ?>
	...
	<div class="jupiterx-widgets-admin-page">
		<button
			...
			<?php echo esc_html( __( 'Add Custom Sidebar', 'jupiterx' ) ); ?>
		</button>
	</div>
