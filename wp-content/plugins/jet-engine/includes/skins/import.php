	...
	class Jet_Engine_Skins_Import {
		...
		public function process_import() {
			...
			if ( ! current_user_can( 'import' ) ) {
				wp_die( __( 'You don\'t have permissions to do this', 'jet-engine' ) );
			}

			if ( empty( $_FILES ) ) {
				wp_die( __( 'File not passed', 'jet-engine' ) );
			}

			$file = $_FILES['file'];

			if ( 'application/json' !== $file['type'] ) {
				wp_die( __( 'Format not allowed', 'jet-engine' ) );
			}
			...
			if ( ! $content ) {
				wp_die( __( 'No data found in file', 'jet-engine' ) );
			}
			...
			set_transient( 'jet_engine_skin_import_log', $this->log );

			wp_redirect( add_query_arg(
				array(
					'page'     => jet_engine()->admin_page,
					'imported' => true,
				),
				admin_url( 'admin.php' )
			) );
		...
		public function get_controls() {
			...
			ob_start();
			?>
			<div class="jet-engine-import">
				<button type="button" class="cx-button cx-button-normal-style" id="jet_engine_import_skin">
					<?php _e( 'Import skin', 'jet-engine' ) ?>
				</button>
				&nbsp;&nbsp;&nbsp;
				<span>
					<?php _e( 'Import new post types, taxonomies and listing items from JSON file', 'jet-engine' ) ?>
				</span>
				<form method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data" class="jet-engine-import__form">
					<div class="jet-engine-import__notice"><?php
						_e( 'Choose an JetEngine skin JSON file, and add it to your website', 'jet-engine' );
					?></div>
					<input type="hidden" name="action" value="jet_engine_import_skin">
					<input type="hidden" name="_nonce" value="<?php echo wp_create_nonce( $this->nonce ); ?>">
					<fieldset class="jet-engine-import__fields">
						...
						<button type="submit" class="cx-button cx-button-primary-style">
							<?php _e( 'Import skin', 'jet-engine' ) ?>
						</button>
					</fieldset>
				</form>
				<?php
					...
					$log = get_transient( 'jet_engine_skin_import_log' );

					if ( ! empty( $log ) ) {

						echo '<div class="jet-engine-import__log">';

						printf(
							'<h4 class="jet-engine-import__log-title">%s:</h4> ',
							__( 'Imported', 'jet-engine' )
						);
						...
						foreach ( $log as $log_item ) {
							...
							echo '<div class="jet-engine-import__log-item">';

							printf( '<span class="jet-engine-import__log-label">%s:</span> ', $log_item['label'] );
							printf( '<span class="jet-engine-import__log-list">%s</span>', implode( ', ', $log_item['items'] ) );
						...
						delete_transient( 'jet_engine_skin_import_log' );
					}
