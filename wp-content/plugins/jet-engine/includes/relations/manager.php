...
if ( ! class_exists( 'Jet_Engine_Relations' ) ) {
	 ...
	 */
	class Jet_Engine_Relations extends Jet_Engine_CPT {
		...
		private $_active_realtions         = array();
		...
		function __construct() {
			...
			add_action( 'init', array( $this, 'register_relations' ), 11 );
		...
		public function init_data() {
			require jet_engine()->plugin_path( 'includes/relations/data.php' );
		...
		public function register_relations() {
			...
			foreach ( $relations as $relation ) {
				...
				if ( ! empty( $this->_active_realtions[ $meta_key ] ) ) {
					continue;
				}
				...
				$this->_active_realtions[ $meta_key ] = $relation;
		...
		public function get_meta( $result, $post, $key, $default, $field ) {
			...
			$relation = isset( $this->_active_realtions[ $key ] ) ? $this->_active_realtions[ $key ] : false;
		...
		public function is_relation_key( $meta_key = null ) {
			return isset( $this->_active_realtions[ $meta_key ] );
		}
		...
		public function process_meta( $result, $post_id, $meta_key ) {
			...
			$relation = isset( $this->_active_realtions[ $meta_key ] ) ? $this->_active_realtions[ $meta_key ] : false;
		...
		public function add_menu_page() {
			...
			add_submenu_page(
				...
				esc_html__( 'Posts Relations', 'jet-engine' ),
				esc_html__( 'Posts Relations', 'jet-engine' ),
				'manage_options',
				$this->page_slug(),
		...
		public function register_pages() {
			...
			$default = array(
				'Jet_Engine_CPT_Page_List_Relations' => $base_path . 'list-relations.php',
				'Jet_Engine_CPT_Page_Add_Relation'   => $base_path . 'add-relation.php',
				'Jet_Engine_CPT_Page_Edit_Relation'  => $base_path . 'edit-relation.php',
			);
			 ...
			 */
			do_action( 'jet-engine/pages/relations/register', $this );
		...
		public function get_current_page() {
			...
			$action = isset( $_GET[ $this->action_key ] ) ? $_GET[ $this->action_key ] : 'list-relations';
