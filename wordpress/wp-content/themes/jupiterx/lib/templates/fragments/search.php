...
function jupiterx_search_has_result() {
	$args = [
		'post_type' => jupiterx_get_option( 'jupiterx_search_post_types', '', [ 'post', 'page', 'portfolio', 'product' ] ),
		'posts_per_page' => -1,
		's' => get_search_query(),
	];
...
function jupiterx_search_product_loop() {
	if ( ! in_array( 'product', jupiterx_get_option( 'jupiterx_search_post_types', '', [ 'product' ] ), true ) || ! class_exists( 'WooCommerce' ) ) {
		return;
	}
