<?php
/**
 * Manage unboarding jupiter theme.
 *
 * @package JupiterX\Framework\API\unboarding
 *
 * @since   1.2.0
 */
...
function jupiterx_activation_popup() {
	...
	$jupiterx_current_version = get_option( 'jupiterx_theme_current_version' ); // prevent showing popup if user had Jupiter X.
	$jupiterx_unboarding      = get_option( 'jupiterx_unboarding_hide_popup' );

	if ( ! jupiterx_had_jupiter() || ! empty( $jupiterx_current_version ) || ! empty( $jupiterx_unboarding ) ) {
		return;
	}

	update_option( 'jupiterx_unboarding_hide_popup', true );
	...
	wp_enqueue_style( 'jupiterx-activation-popup', JUPITERX_API_URL . 'unboarding/assets/css/activation-popup' . JUPITERX_MIN_JS . '.css', [], JUPITERX_VERSION );
	...
	wp_enqueue_script( 'jupiterx-activation-popup', JUPITERX_API_URL . 'unboarding/assets/js/activation-popup' . JUPITERX_MIN_JS . '.js', [], JUPITERX_VERSION, true );
	...
	wp_localize_script( 'jupiterx-activation-popup', 'jupiterx_activation_popup', [
		...
		'images_url'                         => JUPITERX_API_URL . 'unboarding/assets/img/',
