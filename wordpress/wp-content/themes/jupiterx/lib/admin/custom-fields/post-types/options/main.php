...
$post_type = get_post_type( jupiterx_get( 'post' ) ); // @codingStandardsIgnoreLine

if ( is_admin() && empty( $post_type ) && isset( $_GET['post_type'] ) ) { // @codingStandardsIgnoreLine
	$post_type = $_GET['post_type']; // @codingStandardsIgnoreLine
}
...
if ( in_array( $post_type, [ 'post', 'page' ], true ) || empty( $post_type ) ) {
	acf_add_local_field( [
...
// Author box - Post.
if ( ! in_array( $post_type, [ 'portfolio', 'page' ], true ) ) {
	acf_add_local_field( [
		'key'           => "{$key}_author_box",
		'parent'        => $parent,
